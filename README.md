...
4. ✅ Mundane Astrology Research ("World Events" in the nav — "mundane" is the technical term,
   meaning worldly/global, not "boring"; renamed in the UI to avoid that confusion) — significators
   per research category (economy, commodities, geopolitics, elections, disasters), live transit
   positions for just those planets, an optional house-reference (your natal Moon or Ascendant),
   and a research-notes journal per category (saved to `localStorage`). No predictions are
   generated — configurations only, same principle as the original spec.
5. ✅ Reports — Half-Year, Yearly, or custom-range reports, generated by a day-by-day scanner
   (`netlify/functions/scan.js`) that detects sign ingresses, **nakṣatra ingresses** (finer-grained
   than sign changes — useful for KP-style precision), retrograde/direct stations, conjunctions
   (within 1°), combustion onset/end, and eclipses (via `astronomy-engine`'s real eclipse search,
   cross-checked against known 2026 eclipse dates). Filterable by event type and by planet.
   "Download as PDF" uses the browser's native Print dialog (no PDF library dependency) —
   choose "Save as PDF" as the destination.
   Known precision limit: the scanner samples once per day (00:00 UTC), so an event's exact hour
   can be off by up to a day; fine for a report's granularity, not for split-second timing.
